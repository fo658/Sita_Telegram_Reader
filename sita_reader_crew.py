...
def jump_check(chg_direct):
#	'''320(HFE) —> 32L(SHA) :'''
#	'''73V(KN) —> 73S(SHA)'''
	if ('—>' in chg_direct):
		if (chg_direct[0]!=chg_direct[chg_direct.index('>')+2]):
			return True
		elif (chg_direct[1]!=chg_direct[chg_direct.index('>')+3]):
			return True
		else:
			return False
	else:
...
#读取这样的信息，把它们合并成标准格式
	...
	if re.match(r"^.+ CNL RPT CNL.?",info,re.I)!=None:#取消类型
		...
		from_type=two_types[1].strip()
		to_type=two_types[0].strip()
...
# 'MU2805/6 NKG-CTU-NKG': {datetime.date(2020, 12, 13): 'UN PSGR'}, 'MU2853/4 NKG-LHW-NKG': {datetime.date(2020, 12, 15): 'UN PSGR'}}}
	reason_chinese={'ADD':'新增','CNL':'取消','TM_CHG':'改时刻','CHG':'改机型','Route_CHG':'改航路'}
#	print(ultimate)
	for reason in ['ADD','CNL','TM_CHG','CHG','Route_CHG']:
		...
		if len(ultimate[reason])!=0:  #ultimate表单中，确实存在CHG的内容
			show_str+='      \n'
			show_str+='■■■■■■■\n'
			show_str+=reason_chinese[reason]
			show_str+='\n'
			show_str+='■■■■■■■\n'
...
#			print(show_str)
	...
	for piece in show_str.split('$'):
		if jump_check(piece)==False:
			st.insert(tk.END,piece,'out')
		else:
			st.insert(tk.END,piece,'highlight_out')



#	st.insert(tk.END,show_str,'out')
	st.tag_config('out',font=("Fixdsys",13))
	st.tag_config('highlight_out',font=("Fixdsys",13),foreground='red')
